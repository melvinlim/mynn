...
void forwardGPU(const int m,const int n,const double *A, const double *x, double *y, double *deriv){
    ...
    cudaMalloc(&d_x,xSz);
    cudaMemcpy(d_x,x,xSz,cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(y,d_y,ySz,cudaMemcpyDeviceToHost);
		cudaMemcpy(deriv,d_deriv,ySz,cudaMemcpyDeviceToHost);
...
__global__ void weightKernel(const int m,const int n,double *A,const double *x,const double *delta){
...
void weightGPU(const int m,const int n,double *A,const double *x,const double *delta){
    size_t size = m * n * sizeof(double);
		double *d_A=(double *)malloc(size);
    cudaMalloc(&d_A,size);
    cudaMemcpy(d_A,A,size,cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(d_x,x,xSz,cudaMemcpyHostToDevice);
		...
		size_t ySz=m*sizeof(double);
		double *d_delta=(double *)malloc(ySz);
    cudaMalloc(&d_delta,ySz);
    cudaMemcpy(d_delta,delta,ySz,cudaMemcpyHostToDevice);

    dim3 dimBlock(BLOCK_SIZE,BLOCK_SIZE);
    dim3 dimGrid((int)ceil((float)m/(float)dimBlock.x),(int)ceil((float)n/(float)dimBlock.y));
    weightKernel<<<dimGrid,dimBlock>>>(m,n,d_A,d_x,d_delta);
